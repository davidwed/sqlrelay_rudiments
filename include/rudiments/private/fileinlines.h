...
RUDIMENTS_INLINE int file::open(const char *name, int flags) {
	return ((fd=::open(name,flags))!=-1 && getCurrentProperties());
}

RUDIMENTS_INLINE int file::open(const char *name, int flags, mode_t perms) {
	return ((fd=::open(name,flags,perms))!=-1 && getCurrentProperties());
}

RUDIMENTS_INLINE int file::create(const char *name, mode_t perms) {
	return open(name,O_CREAT|O_TRUNC|O_RDWR,perms);
}

RUDIMENTS_INLINE size_t	file::create(const char *name, mode_t perms,
...
RUDIMENTS_INLINE int file::exists(const char *filename) {
	struct stat	st;
...
RUDIMENTS_INLINE int file::getPermissions(const char *filename, mode_t *mode) {
	STAT(filename,mode,st_mode)
}

RUDIMENTS_INLINE int file::getPermissions(int fd, mode_t *mode) {
	FSTAT(fd,mode,st_mode)
}

RUDIMENTS_INLINE int file::getOwnerUserId(const char *filename, uid_t *uid) {
	STAT(filename,uid,st_uid)
}

RUDIMENTS_INLINE int file::getOwnerUserId(int fd, uid_t *uid) {
	FSTAT(fd,uid,st_uid)
}

RUDIMENTS_INLINE int file::getOwnerGroupId(const char *filename, gid_t *gid) {
	STAT(filename,gid,st_gid)
}

RUDIMENTS_INLINE int file::getOwnerGroupId(int fd, gid_t *gid) {
	FSTAT(fd,gid,st_gid)
}

RUDIMENTS_INLINE int file::getSize(const char *filename, off_t *size) {
	STAT(filename,size,st_size)
}

RUDIMENTS_INLINE int file::getSize(int fd, off_t *size) {
...
RUDIMENTS_INLINE int file::getLastAccessTime(const char *filename,
								time_t *atime) {
	STAT(filename,atime,st_atime)
}

RUDIMENTS_INLINE int file::getLastAccessTime(int fd, time_t *atime) {
	FSTAT(fd,atime,st_atime)
}

RUDIMENTS_INLINE int file::getLastModificationTime(const char *filename,
								time_t *mtime) {
	STAT(filename,mtime,st_mtime)
}

RUDIMENTS_INLINE int file::getLastModificationTime(int fd, time_t *mtime) {
	FSTAT(fd,mtime,st_mtime)
}

RUDIMENTS_INLINE int file::getLastChangeTime(const char *filename,
								time_t *ctime) {
	STAT(filename,ctime,st_ctime)
}

RUDIMENTS_INLINE int file::getLastChangeTime(int fd, time_t *ctime) {
	FSTAT(fd,ctime,st_ctime)
}


RUDIMENTS_INLINE int file::getDevice(const char *filename, dev_t *dev) {
	STAT(filename,dev,st_dev)
}

RUDIMENTS_INLINE int file::getDevice(int fd, dev_t *dev) {
	FSTAT(fd,dev,st_dev)
}

RUDIMENTS_INLINE int file::getDeviceType(const char *filename, dev_t *devtype) {
	STAT(filename,devtype,st_rdev)
}

RUDIMENTS_INLINE int file::getDeviceType(int fd, dev_t *devtype) {
	FSTAT(fd,devtype,st_rdev)
}

RUDIMENTS_INLINE int file::getInode(const char *filename, ino_t *inode) {
	STAT(filename,inode,st_ino)
}

RUDIMENTS_INLINE int file::getInode(int fd, ino_t *inode) {
	FSTAT(fd,inode,st_ino)
}

RUDIMENTS_INLINE int file::getNumberOfHardLinks(const char *filename,
							nlink_t *nlink) {
	STAT(filename,nlink,st_nlink)
}

RUDIMENTS_INLINE int file::getNumberOfHardLinks(int fd, nlink_t *nlink) {
