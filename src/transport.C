...
ssize_t	transport::read(char **buffer, char *terminator) {
	...
	*buffer=new char[buffersize];
	...
	while (1) {
		...
		if ((sizeread=safeRead(filedescriptor,
				(void *)&charbuffer,sizeof(char)))==-1) {
			totalread=-1;
			break;
		}
...
ssize_t	transport::safeRead(int fd, void *buf, ssize_t count) {
...
ssize_t	transport::safeWrite(int fd, const void *buf, ssize_t count) {
