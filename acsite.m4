...
then
	...
	then
		...
		then
			...
			AC_MSG_CHECKING(for CURLINFO_ACTIVESOCKET)
			FW_TRY_LINK([#include <curl/curl.h>],[int o=CURLINFO_ACTIVESOCKET;],[$CPPFLAGS $LIBCURLINCLUDES],[$LIBCURLLIBS],[],[AC_DEFINE(RUDIMENTS_HAS_CURLINFO_ACTIVESOCKET,1,libcurl has CURLINFO_ACTIVESOCKET) AC_MSG_RESULT(yes)],[AC_MSG_RESULT(no)])
		fi
	fi
